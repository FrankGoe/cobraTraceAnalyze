...
function calculateTickIntervalLog(businessDelta, screenDelta, tickInterval, forceTickInterval, axisDivisionFactor, multipliers, allowDecimals) {
    ...
    if (!forceTickInterval || !tickInterval) {
        if (interval > 0) {
            result = multipliers.concat(10 * multipliers[0]).reduce(function(r, m) {
...
function calculateTickIntervalDateTime(businessDelta, screenDelta, tickInterval, forceTickInterval, axisDivisionFactor, multipliers, allowDecimals) {
    var result, factor, key, interval = getIntervalByFactor(businessDelta, screenDelta, axisDivisionFactor);
...
function generator(options, getBusinessDelta, calculateTickInterval, calculateMinorTickInterval, getMajorTickIntervalByCustomTicks, getMinorTickIntervalByCustomTicks, convertTickInterval, calculateTicks, calculateMinorTicks, processScaleBreaks) {
    ...
    function generateMajorTicks(ticks, data, businessDelta, screenDelta, tickInterval, forceTickInterval, customTicks, breaks) {
        ...
        var majorTicks = calculateTicks(data.min, data.max, tickInterval, options.endOnTick, breaks.filter(function(b) {
            return b.gapSize
        }), breaks);
...
function logarithmicGenerator(options) {
    ...
    return generator(options, getBusinessDeltaLog(base), calculateTickIntervalLog, calculateMinorTickInterval, getTickIntervalByCustomTicks(log, getValue), getTickIntervalByCustomTicks(getValue, getValue), getValue, calculateTicks(addIntervalLog(base), options.endOnTick ? floor : ceil), calculateMinorTicks(updateTickInterval, addInterval, floor, ceilNumber, ceil), getScaleBreaksProcessor(getValue, log, function(value, correction) {
